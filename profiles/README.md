...
本目錄提供 Exporter 選擇 Dell ECS adapter 的 machine-readable 契約。Profile 描述
版本範圍、transport 差異、capability、已知限制與驗證狀態。

## Profiles

| Profile | Version range | Evidence status |
|---|---|---|
...
`evidence.shared_validated_capabilities`。
...
capability。共用功能狀態與完整 exact-build Profile certification 仍是兩個獨立狀態。
